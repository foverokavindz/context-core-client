---
name: web-ui-mui
description: Build or change React interfaces with Material UI using the project's theme, layout, form, feedback, navigation, and MUI X patterns. Use when a task involves MUI components, `sx`, `styled`, slots, theme configuration, responsive layouts, data grids, or date pickers.
---

# MUI (Material UI) Patterns

> **Quick Guide:** MUI provides pre-styled React components implementing Material Design. Use `createTheme` + `ThemeProvider` for global theming, `sx` for one-off styles, `styled()` for reusable styled components, and `slots`/`slotProps` for inner-element customization. This project uses Material UI and MUI Icons v9. Check installed package versions before applying version-specific guidance.
...
> **All code must follow the project conventions in `AGENTS.md` and the existing codebase.**
...
**(You MUST use `slots`/`slotProps` for component inner-element customization -- deprecated `components`/`componentsProps` APIs were removed in v9)**
...
- Slots/slotProps for component customization
...
**MUI v9 Project Baseline:**

- Deprecated component APIs have been removed; use `slots` and `slotProps`.
- Deprecated system props on `Box`, `Grid`, and `Stack` have been removed; put system styles in `sx`.
- `Grid` no longer supports column directions; use `Stack` for vertical layouts.
- Material UI and MUI X share the v9 major version.
- This project currently uses Emotion as its styling engine.
...
MUI v9 supports CSS variables natively. Use `colorSchemes` for flicker-free light/dark mode.
...
In MUI v9, `slots` and `slotProps` are the standardized API for customizing component inner elements. Older deprecated component-prop APIs have been removed.
...
MUI provides `Box`, `Stack`, `Grid`, and `Container` for layout. Use `Stack` rather than a column-direction `Grid` for vertical layouts.

```typescript
// Grid: 12-column responsive layout (use the size prop)
...
**React 19:** Supported by the project's Material UI v9 packages.
...
| ---------------------- | ----------------------------------------- | ------- |
| `@mui/material`        | Core components (Button, TextField, etc.) | v9.x    |
| `@mui/system`          | sx prop, styled(), ThemeProvider          | v9.x    |
| `@mui/icons-material`  | Material Design icons                     | v9.x    |
| `@mui/x-data-grid`     | DataGrid (free tier)                      | v9.x    |
| `@mui/x-data-grid-pro` | DataGrid Pro                              | v9.x    |
| `@mui/x-date-pickers`  | DatePicker, TimePicker                    | v9.x    |
| `@mui/x-charts`        | Bar, Line, Pie, Scatter charts            | v9.x    |
| `@mui/x-tree-view`     | TreeView                                  | v9.x    |
| `@mui/material-nextjs` | Next.js SSR integration                   | v9.x    |
...
- Using removed `components`/`componentsProps` APIs instead of `slots`/`slotProps`
- Using legacy Grid APIs instead of the current `Grid` `size` prop
...
- Deep importing beyond one level: `@mui/material/styles/createTheme` (unsupported package internals)
...
- `Grid` uses the `size` prop; use `Stack` for vertical layouts because column directions were removed in v9
